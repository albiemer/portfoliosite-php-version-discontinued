<?php
	...
	$_SESSION['VENVWITHPYTHON3ADTHONNY'] = $venvwithpython3andthonny
			...
			<?php 
				...
				if (isset($_SESSION['LOGINEMAIL'])){
					...
					echo '<div id="fb-root" class = "fb-root" >
						<a style = "text-decoration: none; font-size: 100%; color: white; position: center;" href = "../home.php">
						<img style = "border-radius:50px; height: 15%; width: 3%; " src="' . "../Useraccount/" . $email_s . "/" . $profile_s . '" alt="Trusted online service review">' . " " .
						$fname_s . " " . $lname_s . '</a> | 
						<a style = "text-decoration: none; font-size: 80%; color: white; position: center;" href = "bloggs.php#comment_form">Write Comment</a>
						<form action = "logout.php" style = "float: right;"><input type = "submit" name = "submit" style = "font-size: 10px;" method = "post" value = "Logout" = ></form>
					<hr style = "color: yellow;"></div>';
				
					$maincomment_form = '<form id = "comment_form" action = "commentproc.php" method = "post">
				
						<textarea style = "width: 98.5%;" id = "textarea" onKeyDown="limitText(this.form.textarea,this.form.countdown,1000);"   class = "commentbox" cols = "50" name = "MAINCOMMENT" placeholder = "Enter your comment here..." required></textarea><br>
			...
			<section class = "container">
				<article class = "center">
					...
					include("dbinclude.php");
							...
							$comment_read = "SELECT * FROM $blogscontent ORDER BY ID";
							...
							if ($result->num_rows>0){
								...
								while($row = $result->fetch_assoc()){
									...
									if ('albiemer' == $row["Name"] && 'albiemer.porte@yahoo.com' == $email_s){
										...
										echo '<br><br><section id = "' . $nameidcomment . '" class = "comment_style">' . $row["Usertype"] . ':&nbsp&nbsp&nbsp' . $row["Name"] . '<hr><section class = "comment_head"><img class = "comment_image" src="' . "../Useraccount/" . $row["Myprofile"] . '" alt="Trusted online service review">' . $row["Date"] . '<br>' . $row["Weekdays"] . '<br>' . $row["Time"] . '</section>';
										...
										echo '<br><section id = "' . $nameidcomment . '" class = "comment_style">' . $row["Usertype"] . ':&nbsp&nbsp&nbsp' . $row["Name"] . '<hr><section class = "comment_head"><img class = "comment_image" src="' . "../Useraccount/" . $row["Myprofile"] . '" alt="Trusted online service review">' . $row["Date"] . '<br>' . $row["Weekdays"] . '<br>' . $row["Time"] . '</section>';
										echo '<textarea id = "comment_content" class = "comment_content" disabled name = "textareacomment" resize = "none" rows = "12" cols = "10">' . $row["Comment"] . '</textarea></section>';
							...
							<ul>
								...
								<li>
									...
									<div class="content">
										...
										&#8680<a href="#" class="icon-lock">PHP</a><br>
										&#8680<a href="../PDF-Books/Introduction-vb.net-part1.php" class="icon-lock">VB.net Programming</a>
									</div>
								</li>
