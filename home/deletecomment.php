...
if (isset($_SESSION['LOGINEMAIL'])){
	...
	if (isset($_POST['NAME']) && isset($_POST['EMAIL']) && isset($_POST['ID']) ){
		...
		if($email_s == $email){
			...
			$select_name = "Select * FROM guest_commentv2 WHERE Email = '$email' AND Id = '$id' ";
		...
		if($name == $name_r && $email == $email_r && $id == $id_r){
			...
			$delete_comment = "DELETE FROM guest_commentv2 WHERE Email = '$email' And Id = '$id' ";

			if ($conn->query($delete_comment)===TRUE){
				
				header("Location:index.php#displaycomment");
			}
				...
				header("Refresh: 3; URL=http://localhost/index.php#" . $nameidcomment);
